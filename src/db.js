...
async function normalizeOrderTimesToBeijing() {
  const configured = Number(process.env.ORDER_EXPIRE_MINUTES || 30);
  const expireMinutes = Number.isFinite(configured) && configured > 0
    ? Math.floor(configured)
    : 30;

  // 旧版本中 SQLite created_at 使用 UTC，而 Node 生成的 expires_at 使用服务器本地
  // 北京时间，导致两者相差“有效期 + 8 小时”（默认 510 分钟）。只修正这种特征
  // 明确的旧记录，因此该迁移可以在每次启动时安全、幂等地执行。
  await run(
    `UPDATE orders
     SET created_at = datetime(created_at, '+8 hours'),
         paid_at = CASE
           WHEN paid_at IS NULL THEN NULL
           ELSE datetime(paid_at, '+8 hours')
         END,
         delivered_at = CASE
           WHEN delivered_at IS NULL THEN NULL
           ELSE datetime(delivered_at, '+8 hours')
         END
     WHERE ABS(
       (julianday(expires_at) - julianday(created_at)) * 1440
       - (? + 480)
     ) < 5`,
    [expireMinutes]
  );
...
async function initDb() {
  ...
  await normalizeOrderTimesToBeijing();
  ...
  const adminCount = await get('SELECT COUNT(*) AS total FROM admins');
  if (!adminCount.total) {
    ...
    if (process.env.NODE_ENV === 'production' && configuredPassword.length < 8) {
      throw new Error('首次生产部署必须在 .env 中设置至少 8 位的 ADMIN_PASSWORD。');
    }
