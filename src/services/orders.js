...
async function createOrder(productId, customerToken = '', userId = null) {
  ...
  const expireMinutes = Number(process.env.ORDER_EXPIRE_MINUTES || 30);
  const expiresAt = dayjs().add(expireMinutes, 'minute').format('YYYY-MM-DD HH:mm:ss');
  ...
  try {
    ...
    if (existing) {
      ...
      const result = await run(
        `INSERT INTO orders
         (order_no, product_id, sku_id, base_amount, pay_amount, customer_token, user_id, delivery_token, expires_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          orderNo(),
          productId,
    ...
    if (error && error.code === 'SQLITE_CONSTRAINT') {
      const existing = await get(
        `SELECT id
         FROM orders
         WHERE user_id = ? AND product_id = ? AND status IN ('pending', 'review')
...
function isOrderExpired(order) {
  return !order || dayjs(order.expires_at).isBefore(dayjs());
}
...
async function markPaid(orderId, proofId = null) {
  ...
  try {
    await run(
      `UPDATE orders
       SET status = 'paid', paid_at = CURRENT_TIMESTAMP, delivered_at = CURRENT_TIMESTAMP
